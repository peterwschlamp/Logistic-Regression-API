...
LogisticRegression.prototype.train = function (settings) {
  ...
  var lr = 0.1,
    epochs = 200;
  if (typeof settings['input'] !== 'undefined') self.x = settings['input'];
  ...
  if (self.settings['log level'] > 0) console.log("LogisticRegression Final Cross Entropy : ", self.getReconstructionCrossEntropy());
};
...
LogisticRegression.prototype.getReconstructionCrossEntropy = function () {
  ...
  var b = math.mulMatElementWise(math.mulMatScalar(math.addMatScalar(self.y, -1), -1), math.activateMat(math.mulMatScalar(math.addMatScalar(probYgivenX, -1), -1), Math.log));
