...
var MLP = module.exports = function (settings) {
    ...
    for (i = 0; i < self.nLayers + 1; i++) {
        ...
        if (i == 0) inputSize = settings['n_ins'];else inputSize = settings['hidden_layer_sizes'][i - 1];

        if (i == 0) layerInput = self.x;else layerInput = self.sigmoidLayers[self.sigmoidLayers.length - 1].sampleHgivenV();
...
MLP.prototype.train = function (settings) {
    ...
    for (epoch = 0; epoch < epochs; epoch++) {
        ...
        delta[self.nLayers] = math.mulMatElementWise(math.minusMat(self.y, output), math.activateMat(self.sigmoidLayers[self.nLayers].linearOutput(layerInput[self.nLayers]), math.dSigmoid));
        ...
        for (i = self.nLayers - 1; i >= 0; i--) {
            delta[i] = math.mulMatElementWise(self.sigmoidLayers[i + 1].backPropagate(delta[i + 1]), math.activateMat(self.sigmoidLayers[i].linearOutput(layerInput[i]), math.dSigmoid));
        }
        // Update Weight, Bias
        for (var i = 0; i < self.nLayers + 1; i++) {
            var deltaW = math.activateMat(math.mulMat(math.transpose(layerInput[i]), delta[i]), function (x) {
                return 1. * x / self.x.length;
            });
    ...
    if (self.settings['log level'] > 0) console.log("MLP Final Cross Entropy : ", self.getReconstructionCrossEntropy());
};
